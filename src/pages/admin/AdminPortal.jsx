...
export const AdminProducts = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Delete this listing from the catalog?')) {
      const dbProds = JSON.parse(localStorage.getItem('db_products') || '[]');
      const filtered = dbProds.filter(p => p.id !== id);
      localStorage.setItem('db_products', JSON.stringify(filtered));
      addToast('Listing moderation complete', 'info');
      loadAllProducts();
    }
  };
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                ...
                <td style={{ padding: '16px' }}>
                  <button onClick={() => handleDelete(p.id)} style={{ color: '#EF4444', fontSize: '13px', fontWeight: '600' }}>Delete Listing</button>
                </td>
              </tr>
            ))}
          </tbody>
